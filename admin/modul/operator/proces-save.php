...
error_reporting(0);
$var_firstname=htmlentities($_POST['frm_firstname']); //htmlentities() sebuah fungsi untuk merubah semua tag HTML menjadi text biasa
$var_lastname=htmlentities($_POST['frm_lastname']);
...
$sql_check_nis = "
 ...
 if($error != 0){
   ...
   */
$query1="INSERT INTO tbl_person('','person_firstname','person_lastname')VALUES('$var_firstname', '$var_lastname')";

$query2="INSERT INTO tbl_user (username, password, person_id,role_id) VALUES ('$var_username', '$var_password', (SELECT `person_id` FROM `tbl_person` WHERE `person_firstname` = '$var_firstname' AND `person_lastname` = '$var_lastname'), (SELECT `role_id` FROM `ref_role` WHERE `role_name` = 'admin'))";

          // echo $query;
          // exit;
$result=mysql_query($query1, $query2);

if ($result){
    // $derektori="../uploads/silabus/".$nama_file;
