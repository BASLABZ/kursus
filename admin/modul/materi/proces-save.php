...
error_reporting(0);
$var_coursematerial=htmlentities($_POST['coursematerial_title']); //htmlentities() sebuah fungsi untuk merubah semua tag HTML menjadi text biasa

$var_date=date("Y-m-d, H:i");
//-----------------------ambil gambar
$type_file  = $_FILES['coursematerial_file']['type'];
$lokasi_file =   $_FILES['coursematerial_file']['tmp_name'];
$nama_file =     $_FILES['coursematerial_file']['name'];
$ukuran_file =   $_FILES['coursematerial_file']['size'];

//-------------------ambil video

$type_filev  = $_FILES['coursematerial_video']['type'];
$lokasi_filev =   $_FILES['coursematerial_video']['tmp_name'];
$nama_filev =     $_FILES['coursematerial_video']['name'];
$ukuran_filev =   $_FILES['coursematerial_video']['size'];
...
$query="INSERT INTO tbl_coursematerial VALUES 
        ('',
            '".$var_coursematerial."',
            '".$var_date."',
            '".$nama_file."',
...
if ($result){
    $derektori="../uploads/materi_kursus/".$nama_file;
